...
url = "http://www.espnfc.com/club/ac-milan/103/squad"
r = requests.get(url)
...
for i,j in zip(table1,table):
    ...
    post['mI']['person']={'profession':'sports_player'}
    post['mI']['footballer']={'attribute':attribute[x]}
    post['mI']['footballer']['club']=['ac milan']
    print post
    try:
        #pass
        coll.insert(post)
...
#post['_id']=getEntityHash(name.strip().lower())
post['EA_H']=[]
post['T']=['person']
post['mI']=defaultdict(list)
#post['mI']['person']={'nationality':'french'}
post['mI']['person']={'working_at':'ac milan'}
